...
import com.bradypod.common.dao.MyBatisBaseDAO;
...
public class MyBatisBaseService<E extends Serializable, T extends BaseMapper<E>> implements
		BaseService<E, T> {
	...
	public MyBatisBaseService() {
		Class<?> c = (Class<?>) this.getClass();
		Type t = c.getGenericSuperclass();
		if (t instanceof ParameterizedType) {
			this.clazz = (Class<T>) ((ParameterizedType) t).getActualTypeArguments()[1];
		}
	}
	...
	@Override
	public T getMapper() {
		return myBatisBaseDAO.getSqlSession().getMapper(clazz);
	}
	...
	public int save(E e) {
		...
		if (rows != 1) {
			throw new ServiceException(ResultCode.DATABASE_ERROR, "插入数据失败, 期望插入1行, 结果为" + rows
					+ "," + e.toString());
		}
		return rows;
	}
