...
public class TestLock {
	public static void main(String[] args) {
		...
		User u3 = new User("张三他弟", myCount, -6000, lock, false);
		User u4 = new User("张三", myCount, -6000, lock, false);
		User u5 = new User("张三他爷", myCount, 0, lock, true);
		// 在线程池中执行各个用户的操作
		pool.execute(u1);
		pool.execute(u2);
		pool.execute(u3);
...
class User implements Runnable {
	...
	public void run() {
		if (ischeck) {
			...
			try {
				if ("张三他爹".equals(name)) {
					throw new RuntimeException("张三他爹弄错了");
				}
				// 执行现金业务
