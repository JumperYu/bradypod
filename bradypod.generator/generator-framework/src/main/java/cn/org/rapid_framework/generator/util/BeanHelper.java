...
public class BeanHelper {
			...
			if (targetPd.getWriteMethod() != null
				...
				try {
					if (source instanceof Map) {
						...
						PropertyDescriptor sourcePd = getPropertyDescriptors(source.getClass(),
								targetPd.getName());
					...
					throw new IllegalArgumentException("Could not copy properties on:"
	...
	private static void setProperty(Object target, PropertyDescriptor targetPd, Object value)
		...
		writeMethod.invoke(target,
				new Object[] { convert(value, writeMethod.getParameterTypes()[0]) });
	}
	...
	private static Object convert(String value, Class<?> targetType) {
		...
		if (DateHelper.isDateType(targetType)) {
			return DateHelper.parseDate(value, targetType, "yyyyMMdd", "yyyy-MM-dd",
					"yyyyMMddHHmmSS", "yyyy-MM-dd HH:mm:ss", "HH:mm:ss");
		}

		throw new IllegalArgumentException("cannot convert value:" + value + " to targetType:"
				+ targetType);
	}
}
