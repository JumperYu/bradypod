...
/**
 * 默认会话管理器实现 Created by oldmanpushcart@gmail.com on 15/5/2.
 */
public class DefaultSessionManager implements SessionManager {

	// 5分钟
	private static final long DURATION_5_MINUTE = 1L * 60 * 1000;
	...
	@Override
	public Session newSession(int javaPid, SocketChannel socketChannel,
			Charset charset) {
		final int sessionId = sessionIndexSequence.getAndIncrement();
		final Session session = new Session(javaPid, sessionId,
				DEFAULT_SESSION_DURATION, socketChannel, charset) {
			@Override
			public void destroy() {
				super.destroy();
	...
	private void activeSessionExpireDaemon() {
		final Thread sessionExpireDaemon = new Thread(
				"ga-session-expire-daemon") {
			...
			public void run() {
				while (!isDestroyRef.get() && !isInterrupted()) {
					...
					for (final Map.Entry<Integer, Session> entry : sessionMap
							.entrySet()) {
						...
						if (null == session || session.isExpired()) {
							...
							LogUtil.log("session[{}] was expired");
							...
							if (null != session) {
								...
								try {
									...
									session.getSocketChannel()
											.write(ByteBuffer
													.wrap("Session timed out.\n".getBytes(session
															.getCharset())));
								} catch (IOException e) {
									LogUtil.log("write expired message to session[{}] failed.");
								}
	...
	public void clean() {
		...
		LogUtil.log("session manager clean completed.");
	...
	public void destroy() {
		...
		LogUtil.log("session manager destroy completed.");
